...
cursor.execute('Select * from talabat_restaurants_urls where page_active is null')
c = 0
...
for row in cursor.fetchall():
    ...
    for i, line in enumerate(html_element):
        if str(line).find(text) > 1:
            update_command = 'Update talabat_restaurants_urls Set [page_active] = 0 where id = ' + str(row[0])
    ...
    if page_not_found == 0:
        ...
        update_command = 'Update talabat_restaurants_urls Set [done] = 1, [page_active] = 1 where id = ' + str(row[0])
        cursor.execute(update_command)
        conn.commit()
